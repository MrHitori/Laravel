...
class AuthController extends Controller
{
        ...
        catch (NotActivatedException $e)
        {
            ...
            if ($sent === 0)
            {
                return Redirect::to('login')
                    ->withErrors('Error sending activation letter.');
            }
            $errors = 'Your account is not ativirovan! Look in your inbox an email with a link to activate (you sent a further letter).';
            return view('auth.login')->withErrors($errors);
        }
        catch (ThrottlingException $e)
        {
            $delay = $e->getDelay();
            $errors = "Your account has been suspended for {$delay} seconds.";
        }
        return Redirect::back()
            ->withInput()
            ->withErrors($errors);
    ...
    public function registerProcess(Request $request)
    {
        ...
        if ($sentuser = Sentinel::register($input))
        {
            ...
            return Redirect::to('login')
                ->withSuccess('Your account has been created. Check to activate Email.')
    ...
    public function resetOrderProcess(Request $request)
    {
        ...
        if ( ! $sentuser)
        {
            return Redirect::back()
                ->withInput()
                ->withErrors('User with this E-Mail has not been selected.');
        }
        ...
        $sent = Mail::send('mail.account_reminder', compact('sentuser', 'code'), function($m) use ($sentuser)
        {
            ...
            $m->to($sentuser->email)->subject('Password reset');
        });
        if ($sent === 0)
        {
            return Redirect::to('reset')
                ->withErrors('Error sending email.');
        }
    ...
    public function resetCompleteProcess(Request $request, $id, $code)
    {
        ...
        if ( ! $user)
        {
            return Redirect::back()
                ->withInput()
                ->withErrors('User doesn`t exist.');
        }
        if ( ! Reminder::complete($user, $code, $request->password))
        {
            return Redirect::to('login')
                ->withErrors('Invalid or expired password reset code.');
        }
        return Redirect::to('login')
